...
# Read. Get the two existing rows
...
# Add new values
new_dat = tibble(location_type_id = c("f88960aa-9504-47c1-957c-5acef2c84e6c",
                                      "6f08ebe8-579e-4181-9bae-ba9cff661114",
                                      "8668a1fe-3d0c-4b2b-beb4-f606a0dc78e1",
                                      "315b46b4-ff59-44f1-8d57-65c839f2e25b",
                                      "a34fbaa8-6a72-4df9-aded-0b4e85989766"),
...
# Create LUT. Some example protocol types. Probably need others. 
new_dat = tibble(protocol_type_id = c("e6ba654d-34e6-476b-a054-2f644aa7cf27",
                                      "f206c0e5-cac0-4cbd-9d24-d5639d269017",
                                      "ad8e034d-64ed-4e7b-8b51-4fa589a3135c"),
...
# Create LUT. May need others. Invertebrates are so much more interesting.
new_dat = tibble(sex_id = c("6d2823f7-5729-4333-96df-0a907adc54dc",
                            "fc84bdc4-7be1-4126-9ef9-c0f3a4d594ab",
                            "db372d82-3a88-49e7-92d7-9a787c9a5f33",
                            "4fafbe94-5aff-4f28-a288-13b0c7a11676",
                            "d1c67348-3181-454b-a6a2-1b777e7914f8"),
...
# Create LUT
new_dat = tibble(season_type_id = c("e15f7bdf-1b5a-4536-a161-3a33173bd611",
                                    "541dc28d-e8a8-4175-aef9-b794bd4330c9",
                                    "4aafd052-ec4b-4e15-b205-6f12fff5a066"),
...
# Pull out bloc
mloc = dat |>
  mutate(location_type_id = "315b46b4-ff59-44f1-8d57-65c839f2e25b") |>    # Management Region
  mutate(comment_text = NA_character_) |>
...
# Source of error was that we had tide corrections for beaches where we had no polygons. 
# These are probably from beaches we surveyed prior to 2003. 
...
add_loc = no_loc_id |>
  mutate(location_type_id = "8668a1fe-3d0c-4b2b-beb4-f606a0dc78e1") |>    # Beach polygon, Intertidal management.
  mutate(location_code = NA_character_) |>                                # Should probably add missing polygons for beaches 
  mutate(location_name = local_beach_name) |>                             # where we have corrections but no polygons at some point.
  mutate(location_description = NA_character_) |>                         # These are probably from pre-2003 flight routes.
  mutate(comment_text = NA_character_) |>
